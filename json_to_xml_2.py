...
with open(config.project_dir + 'train/instances_1.json') as update_instances_file:
    update_instances = json.load(update_instances_file)

xml_dir = config.project_dir + 'xml/'

for image_id, instance in update_instances.items():

    image_date = image_id.split('.')[0] + '/'

    current_dir = xml_dir + image_date
    ...
    xml_file_path = annotations_dir + image_id + '.xml'
    ...
    folder_node_text = xml_document.createTextNode(image_date + 'images/')
    ...
    path_node_text = xml_document.createTextNode(
        image_date + 'images/' + image_id + '.jpg')
