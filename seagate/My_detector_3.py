...
class My_detector:
    def __init__(self, num_features=20, num_rows=41, num_cols=48, levels=8, ratio=0.8):
    ...
    def detectAndCompute(self, image, mask=None):
        ...
        for level, (image, mask, scale) in enumerate(zip(image_list, mask_list, scale_list)):
            ...
            eigen_image = cv2.cornerMinEigenVal(image, 3, 3)

            # eigen_image = cv2.cornerEigenValsAndVecs(image,3, 3)[:, :, :2]
            # eigen_image = np.amax(eigen_image, axis=-1)
