...
class MultiHarrisZernike:
    ...
    def detectAndCompute(self, gr_img, mask=None, timing=False):
        ...
        if timing:
            print("Generate pyramid - {:0.4f}".format(time.time() - st))
            st = time.time()
        F = self.feat_extract_p2(P)
        if timing:
            print("Extract features - {:0.4f}".format(time.time() - st))
            st = time.time()
        Ft = self.feat_thresh_sec(F, *gr_img.shape)
        if timing:
            print("Feature Threshold - {:0.4f}".format(time.time() - st))
            st = time.time()
        JA, JB = self.z_jet_p2(P, Ft)
        if timing:
            print("Feature jets - {:0.4f}".format(time.time() - st))
            st = time.time()
    ...
    def detect(self, gr_img, mask=None, timing=False):
        ...
        kp = [cv2.KeyPoint(x, y, self.zrad * (sc + 1) * 2, angle=ang, response=res, octave=sc)
              for x, y, ang, res, sc in zip(Ft['jvec'], Ft['ivec'], np.rad2deg(alpha),
                                            Ft['evec'], Ft['svec'])]
        if timing:
            print("Keypoint export - {:0.4f}".format(time.time() - st))
            st = time.time()
