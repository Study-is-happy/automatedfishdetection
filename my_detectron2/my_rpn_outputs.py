...
from my_detectron2.my_sampling import subsample_labels
...
def my_find_top_rpn_proposals(
    ...
    num_anchors_per_map = [np.prod(x.shape[1:])
                           for x in pred_objectness_logits]
    gt_objectness_logits = torch.split(
        gt_objectness_logits, num_anchors_per_map, dim=1)

    for level_id, proposals_i, logits_i, gt_logits_i in zip(
        itertools.count(), proposals, pred_objectness_logits, gt_objectness_logits
    ):
...
class my_RPNOutputs(object):
    def __init__(
        ...
        self.cls_loss_factor = cls_loss_factor
    ...
    def _get_ground_truth(self):
        ...
        predict_objectness_logits = torch.sigmoid(cat(
            self.predict_objectness_logits(), dim=1))

        for image_size_i, anchors_i, gt_classes_i, gt_boxes_i, predict_objectness_logits_i in zip(self.image_sizes, anchors, self.gt_classes, self.gt_boxes, predict_objectness_logits):
            """
            ...
            if len(gt_boxes_i) == 0:
                ...
                # gt_objectness_logits_i[top_index] = -1
    ...
    def losses(self):
        ...
        gt_objectness_logits = cat([x.flatten()
                                    for x in gt_objectness_logits], dim=0)
        ...
        gt_anchor_deltas = torch.split(
            gt_anchor_deltas, num_anchors_per_map, dim=1)
        # Concat from all feature maps
        gt_anchor_deltas = cat([x.reshape(-1, B)
                                for x in gt_anchor_deltas], dim=0)
        ...
        self.batch_size_per_image = torch.sum(
            gt_objectness_logits != -1).item()
    ...
    def predict_proposals(self):
        ...
        for anchors_i, pred_anchor_deltas_i in zip(anchors, self.pred_anchor_deltas):
            ...
            pred_anchor_deltas_i = (
                pred_anchor_deltas_i.view(
                    N, -1, B, Hi, Wi).permute(0, 3, 4, 1, 2).reshape(-1, B)
            )
