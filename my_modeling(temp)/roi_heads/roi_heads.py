...
class ROIHeads(torch.nn.Module):
    ...
    def _sample_proposals(self, matched_idxs, matched_labels, gt_classes):
        ...
        if has_gt:
            ...
            num_gt = torch.nonzero(
                gt_classes != self.num_classes).squeeze(1).numel()

            gt_classes = gt_classes[matched_idxs]

            background_index = torch.nonzero((gt_classes == self.num_classes) & (
                matched_labels == 1)).squeeze(1)
            ...
            gt_classes[background_index] = self.num_classes+1

            fg_index = torch.nonzero((gt_classes != self.num_classes)
                                     & (gt_classes != -1)).squeeze(1)

            num_fg = fg_index.numel()

            fg_value = gt_classes[fg_index]

            gt_classes[:(num_fg-num_bg-num_gt)*2] = -1

            gt_classes[fg_index] = fg_value
        ...
        if has_gt:
            gt_classes[background_index] = self.num_classes
