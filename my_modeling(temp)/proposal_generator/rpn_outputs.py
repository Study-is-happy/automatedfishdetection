...
class RPNOutputs(object):
        ...
        for image_size_i, anchors_i, gt_classes_i, gt_boxes_i, predict_objectness_logits_i in zip(self.image_sizes, anchors, self.gt_classes, self.gt_boxes, predict_objectness_logits):
            ...
            if len(gt_boxes_i) == 0:
                ...
                _, top_index = predict_objectness_logits_i.sort(
                    descending=True)
                top_index = top_index[:(num_fg-num_bg)*2]
    ...
    def losses(self):
        ...
        def resample(label, background_index):
            """
            ...
            label.scatter_(0, background_index, 0)
            return label
        ...
        gt_objectness_logits = torch.stack(
            [resample(label, background_index) for label, background_index in zip(gt_objectness_logits, background_indexes)], dim=0
        )
